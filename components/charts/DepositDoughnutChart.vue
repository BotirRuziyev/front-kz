...
@Component
export default class DepositDoughnutChart extends Vue {
  @Prop({ default: null }) readonly stepDeposit!: number | null
  @Prop({ default: null }) readonly activeDepositType!: string | null
  @Prop({ default: null }) readonly activeCashedType!: string | null
  ...
  private updateChartBackground() {
    const activeItem = this.chartData?.labels?.find(
      (label: string) =>
        typeof label === 'string' &&
        typeof this.activeCashedType === 'string' &&
        label.toLocaleLowerCase() === this.activeCashedType.toLocaleLowerCase()
    )
    ...
    const activeIndex = this.chartData?.labels?.findIndex(
      (label: string) =>
        typeof label === 'string' &&
        typeof this.activeCashedType === 'string' &&
        label.toLowerCase() === this.activeCashedType.toLowerCase()
    )
  ...
  @Watch('activeDepositType')
  onDepositTypeChanged() {
  ...
  private updateChartOpacity() {
    ...
    meta.data.forEach((_arc: any, index: number) => {
      ...
      const isActive = this.activeDepositType
        ? label === this.activeDepositType?.toLowerCase()
        : false
